...
class MWIUnit(HardwareUnit):
  def __init__(self, name: str, cycle_table: OperationCycleTable, mwi_window_size: int, is_fixed_point: bool):
    latency = cycle_table.sub + cycle_table.add + (cycle_table.shift if is_fixed_point else cycle_table.mul)
    ...
    self.window = CircularDelayLine[int | float](capacity=mwi_window_size, zero_value=zero)
  ...
  def process_token(self, token: SampleToken, current_cycle: int) -> SampleToken:
    ...
    expired_value = self.window.delay(self.mwi_window_size)
