...
class SampleQueue(HardwareUnit):
  ...
  def __init__(self, name: str, queue_size: int, window_size: int = 72, hop_size: int = 36):
    ...
    self.queue: np.ndarray = np.zeros(queue_size, dtype=np.float64)
  ...
  # Extract the oldest window_size samples from the queue
  def get_window(self) -> list:
    if not self.window_ready():
      return []

    read_ptr = (self.write_ptr - self.sample_count) % self.queue_size
  ...
  # True when the queue has room for at least one more sample
  def is_available(self) -> bool:
    return self.sample_count < self.queue_size
